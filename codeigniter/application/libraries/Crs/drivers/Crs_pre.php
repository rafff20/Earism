...
class Crs_pre extends CI_Driver
{
	...
	function explain($product, $usermodel, $pref)
	{
		...
		foreach($product as $id => $prod)
		{
			...
			$txt = 'The Product '.str_replace('_', ' ', $prod['produk']).' is an '.$type.' type audio device with a price '.$this->_num2idr(isset($price[0]) ? $price[0] : 0).' . Which supports needs such as : <ul>';
	...
	function _get_leaf($um)
	{
		...
		// Check if $um is an array before looping through it
		if (is_array($um)) {
			foreach ($um as $u) {
				if (isset($u['leaf']) && $u['leaf']) {
					$result[] = $u;
				}
			}
		} else {
			// Handle the case where $um is not an array (you can log an error or handle it accordingly)
			// For example, you can return an empty array or throw an exception
			// Here, I'm returning an empty array
			return $result;
		}
	...
	function _get_status($um, $status)
	{
		...
		if (is_array($um)) {
			foreach ($um as $u) {
				if (isset($u['status']) && $u['status'] == $status) {
					if (isset($u['name'])) {
						$result[] = $u['name'];
					}
				}
